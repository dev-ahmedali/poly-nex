  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 px-4 pt-4 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl">
        <nav className="relative flex items-center justify-between rounded-2xl border border-white/10 bg-(--surface)/75 px-4 py-3 text-(--text) shadow-[0_10px_40px_rgba(0,0,0,0.35)] backdrop-blur-xl sm:px-5 lg:px-6">
          <Link href="/" className="flex items-center gap-3">
            ...
            <div className="leading-none">
              <p className="text-base font-semibold tracking-wide text-(--text) sm:text-lg">
                Poly<span className="text-(--bright-blue)">nex</span>
              </p>
              <p className="mt-1 hidden text-xs uppercase tracking-[0.22em] text-(--muted) sm:block">
                Motion Portfolio
              </p>
            </div>
          </Link>
          ...
          <AnimatePresence>
            {isOpen && (
              ...
              >
                <div className="overflow-hidden rounded-2xl border border-white/10 bg-(--surface)/95 p-3 shadow-[0_18px_50px_rgba(0,0,0,0.45)] backdrop-blur-xl">
                  <div className="flex flex-col gap-1">
                    {NAV_LINKS.map((item) => (
                      <Link
                        ...
                        className="rounded-xl px-4 py-3 text-sm font-medium text-(--text) transition hover:bg-white/5 hover:text-white"
                      >
                        {item.label}
                      </Link>
                    ))}
